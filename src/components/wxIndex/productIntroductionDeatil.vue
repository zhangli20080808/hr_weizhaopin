<template>
  <div id="productIntroductionDeatil">
    <scroller height="128"  lock-y :scrollbar-x=false width="auto">
        <tab :line-width="1" custom-bar-width="60px" active-color="#5AA2E7" bar-active-color="#5AA2E7">
            <tab-item v-for="(item, inde) in productIntroductionList" :selected="inde == index"  @on-item-click="handler" :key="item.productImageId ">{{item.productName }}</tab-item>
        </tab>
    </scroller>
    

      <!-- <tab :line-width="1" v-if="productIntroductionList.length&&productIntroductionList.length>1" custom-bar-width="60px" active-color="#5AA2E7" bar-active-color="#5AA2E7">
        <tab-item v-for="item in productIntroductionList" @on-item-click="handler" :key="item.productImageId ">{{item.productName }}</tab-item>
    </tab> -->
      <!-- <h3 v-if="productIntroductionList.length&&productIntroductionList.length==1">{{productIntroductionNow.productName}}</h3> -->
      <!-- <div class="hr"></div> -->
      <img :src="productIntroductionNow.productImageUrl" alt="暂无图片">
      <!-- <div class="hr"></div> -->
      <div class="textWrap">
        <b class="title">
          产品描述
      </b>
      <p>{{productIntroductionNow.productDescription}}</p>
      </div>
  </div>
</template>
<script>
import { Tab, TabItem, Sticky, Divider, XButton, Swiper, SwiperItem ,Scroller} from 'vux'
export default {
  data(){
      return{
        productIntroductionList: this.$route.query.productIntroductionList,
        // productIntroductionNow: '',
        productImageUrl: '',
        productName: '',
        productDescription: '',
        index: this.$route.query.index,
      }
  },
  mounted () {
      this.handler();
      this.index = this.$route.query.index;
      console.log(this.$route.query.index)
      console.log('this.$route.query.index')
  },
  computed: {
      productIntroductionNow(){
          return this.productIntroductionList[this.index];
      }
  },
  methods: {
      handler(e){
        if(e){
            this.index = e;
        }else{
            this.index = 0;
        }
      }
  },
   components: {
    Tab,
    TabItem,
    Sticky,
    Divider,
    XButton,
    Swiper,
    SwiperItem,
    Scroller
  },
}
</script>
<style scoped>
h3{
    background-color: #fff;
    font-size: 18px;
    font-weight: 400;
    color: #425062;
    line-height: 1.3;
    text-align: center;
    padding-top: 10px;
}
.hr{
    width: 100%;
    display: inline-block;
    height: .4rem;
    background: #F8F8FC;
}
img{
    width: 100%;
    height: 3.6rem;
    padding: 3%;
}
.title{
    background-color: #fff;
    height:24px; 
    font-size:17px;
    font-family:PingFangSC-Regular;
    color:rgba(0,0,0,1);
    line-height:24px;
}
.textWrap{
    background-color: #fff;
    width: 100%;
    padding: 20px 5%;
}
p{
    margin-top: 20px;
    font-size: 13px;
    line-height: 20px;
    color: #666;
}
.vux-tab .vux-tab-item{
    overflow: hidden;
}
</style>


