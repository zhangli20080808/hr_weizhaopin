<template>
  <div class="interpolateResult" id="interpolateResult">
    ...
    <div class="content">
      ...
      <x-button type="primary" @click.native="verification" class="hrm_primary_btn background-color-5aa2e7" :show-loading="btnLoading1">我已完成认证</x-button>
      <x-button plain @click.native="retransmission" class="hrm_primary_btn" :show-loading="btnLoading1">重新发送邮件</x-button>
      <x-button plain type="default" @click.native="goInterpolate" class="hrm_primary_btn" :show-loading="btnLoading1">重新填写认证信息</x-button>
    </div>
  </div>
</template>
<script>
...
<style scoped>
  .interpolateResult{background-color: #fff;}
  .header{background-color: #5aa2e7;color: #333;}
  .content{text-align: center;}
  .content p{color: #333;width: 75%;margin:0 auto;}
  .hrm_primary_btn{width: 300px;margin-top: 0.68rem!important;outline: none}
  .btn_text{width: 300px;margin: 0 auto;}
  .background-color-5aa2e7{background: #5aa2e7;}
</style>
<style>
  .interpolateResult .weui-btn_default {border-color:#ddd;}
</style>
