<template>
  <div id="add_resume" class="add_resume">
    <div class="first" v-show="type==1">
      <div class="resume_icon"><i class="iconfont">&#xe647;</i></div>
      <p>一分钟创建微简历</p>
      <a @click="go(2)">去创建></a>
    </div>
    <div class="content" v-show="type==2">
      <ul class="content_header">
      	<li :class="{active:headType==1,successd:headType>1}">基本信息<span></span><i class="el-icon-check" v-if="headType>1"></i></li>
      	<li :class="{active:headType==2,successd:headType>2}">联系方式<span></span><i class="el-icon-check" v-if="headType>2"></i></li>
      	<li :class="{active:headType==3,successd:headType>3}">教育经历<span></span><i class="el-icon-check" v-if="headType>3"></i></li>
      	<li :class="{active:headType==4,successd:headType>4}">工作经历<span></span><i class="el-icon-check" v-if="headType>4"></i></li>
      </ul>
      <!--基本信息-->
      <div v-if="headType==1">
      ...
      <div v-if="headType==2">
        <group :label-width="labelWidth">
          <x-input title="邮  箱" name="email" placeholder="请输入邮箱地址" is-type="email" v-model="interviewResumeInfo.email"></x-input>
          <x-input title='手机号码' v-model="interviewResumeInfo.phone"  mask="999 9999 9999" :max="13" is-type="china-mobile" placeholder="请输入手机号码"></x-input>
        </group>
      ...
      <!-- 教育经历 -->
      <div v-if="headType==3">
        <group label-width="100%" v-for="(educationHistory,index) in interviewResumeInfo.educationHistoryList" :key="educationHistory.startDateStr">
          <cell :title="educationHistory.startDateStr+'  至  '+educationHistory.endDateStr" v-if="educationHistory.isReading==0"><i class="el-icon-delete color_F96868" @click="removeEducationSave(index)"></i></cell>
          <cell :title="educationHistory.startDateStr+'  至今'" v-else><i class="el-icon-delete color_F96868" @click="removeEducationSave(index)"></i></cell>
          <p class="side">{{educationHistory.graduateSchool}}&nbsp;&nbsp;{{professionalValue[educationHistory.educationLev-1]}}<br/>{{educationHistory.major}}</p>
        ...
        <group v-if="addEducation" label-align="left" :label-width="labelWidth">
          ...
          <datetime v-model="value4" @on-change="change" title="结束时间" format="YYYY-MM" v-show="!isReading" :min-year="1970" :max-year="2020"></datetime>
      ...
      <!-- 工作经历 -->
      <div v-if="headType==4">
        <group label-width="100%" v-for="(workHistory,index) in interviewResumeInfo.workHistoryList" :key="workHistory.startDateStr">
          <cell :title="workHistory.startDateStr+'  至  '+workHistory.endDateStr" v-if="workHistory.isWorking==0"><i class="el-icon-delete color_F96868" @click="removeExperienceSave(index)"></i></cell>
          <cell :title="workHistory.startDateStr+'  至今'" v-else><i class="el-icon-delete color_F96868" @click="removeExperienceSave(index)"></i></cell>
        ...
        <group v-if="addExperience" :label-width="labelWidth">
          ...
          <datetime v-model="startDateStr" @on-change="change" title="开始时间" format="YYYY-MM" :min-year="1970" :max-year="2020"></datetime>
          <datetime v-model="endDateStr" @on-change="change" title="结束时间" format="YYYY-MM" v-show="!isWorking" :min-year="1970" :max-year="2020"></datetime>
...
<script>
import { XInput,XButton,Group,Picker,Cell,TransferDom,Popup,DatetimeView,PopupPicker,Flexbox, FlexboxItem,XSwitch,Datetime,Icon,Box,Toast  } from 'vux'
  export default {
    name:'add',
    data(){
      return{
...
<style scoped>
  ...
  .content_header li span{display: none;}
  .content_header li.active{color: #fff;}
  .content_header li.successd{color:#fff;}
  .content_header li.active span {width: 0.2rem;height:  0.2rem;background-color: #fff;position: absolute;bottom: -7px;display: block;left: 50%;margin-left: -0.1rem;transform: rotateZ(45deg);}

  .btn{color:#fff;background-color:#5AA2E7;height:0.92rem;line-height:0.2rem;}
  .add_resume .btn:active{color:#fff;background-color:#5AA2E7;}
  .add_resume .btn:focus{outline:none;}
  .position_bottom{position: fixed;bottom:10px;padding:0 15px;}

  .add_resume .last_step{color:#fff;background-color:#C0CCDA;}
  .add_resume .last_step:active{color:#fff;background-color:#C0CCDA;}
  .add_resume .last_step:focus{color:#fff;background-color:#C0CCDA;}
  .side{padding:0px 15px 10px;}
  .add_education{font-size:0.28rem;color:#5aa2e7;padding:10px 15px;}
  .success_resume{font-size:0.32rem;text-align:center;color: #1F2D3D;margin-top:0.68rem;}
  #add_resume .success_resume_btn{width: 3.2rem;margin: 1rem auto 0;display:block;}

  .slide{padding: 10px 15px;}
  .slide li{margin-bottom:10px;}
  .resume_icon{width: 1rem;height: 1rem;color: #fff;background-color: #5aa2e7;text-align: center;vertical-align: middle;border-radius: 10px;margin: 0 auto;}
  .resume_icon i.iconfont{line-height: 1rem;width: 1rem;height:1rem;display: block;font-size: 0.5rem;}
</style>
<style>
  .add_resume .weui-switch:focus{outline:none;outline-offset:0}
  .add_resume a:hover{color:#000;text-decoration:none;}
  .add_resume .color_F96868{color:#F96868;}
  .add_resume .weui-btn{border-radius:4px;}
  .add_resume .weui-cell__ft{text-align:left;}
